...
class Athena_Simple_Jwt_Authentication_Profile {
	/**
	 * The plugin name
	 *
	 * @var string
	...
	public function admin_notices() {
		...
		if ( ! empty( $_GET['revoked'] ) && 'all' === $_GET['revoked'] ) {
			$message = __( 'All tokens have been revoked.', 'athena' );
		} elseif ( ! empty( $_GET['revoked'] ) ) {
			// TranslatorS: %s is a token ID.
			$message = sprintf( __( 'The token %s has been revoked.', 'athena' ), $_GET['revoked'] );
		} elseif ( ! empty( $_GET['removed'] ) ) {
			$message = __( 'All expired tokens have been removed.', 'athena' );
		}
...
new Athena_Simple_Jwt_Authentication_Profile( $plugin_name, $plugin_version );
